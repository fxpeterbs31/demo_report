...
expander_income = st.expander('Income Barang', expanded = True)
...
df_stok_november_total_brand = df_stok_november.groupby(['Brand']).agg(Total = ('Harga Jual', 'sum')).sort_values(by = 'Total').reset_index()
income_kotor = formatrupiah(df_stok_november['Harga Jual'].sum())

expander_income.markdown('#### Total Pendapatan Kotor Barang Depo: :red[{},-]'.format(income_kotor))

if 'stats_income' not in st.session_state:
    st.session_state.stats_income = False

def callback_stats_income():
    st.session_state.stats_income = True

btn_stats_income = expander_income.button('Lihat Detail Income Barang Depo', on_click = callback_stats_income)
if btn_stats_income or st.session_state.stats_income:
    fig = go.Figure(
        go.Bar(
            x = list(df_stok_november_total_brand['Brand']),
            y = list(df_stok_november_total_brand['Total']),
            hovertemplate =
            'Brand: <b>%{x}</b>'+
            '<br>' +
            'Total: <b>%{text}</b><extra></extra>',
            text = [formatrupiah(x) for x in list(df_stok_november_total_brand['Total'])],
            showlegend = False,
            marker_color=['red']*df_stok_november_total_brand.shape[0]
        ),
        layout=dict(
            title=dict(
                text='Pendapatan Kotor Barang <br>Bulan {} {}\n'.format(calendar.month_name[bulan], tahun),
            )
        )
    )
    ...
    expander_income.plotly_chart(fig, use_container_width = True)
